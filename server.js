...
const transporter = nodemailer.createTransport({
  host: 'grid133.mailgrid.com.br',
  port: 587,
  secure: false, // Use TLS
  auth: {
    user: 'stwbrasil@stwbrasil.com',
    pass: 'PuyP4bqcfx',
  },
});

app.post('/send-email', (req, res) => {
  const { nome, contato, data_hora, descricao, ativos, impacto, mitigacao, causa, anonimo, confirmacao, evidencias } = req.body;
  ...
  const mailOptions = {
    ...
    html: `
      ...
      <p><strong>Evidências/Logs:</strong> ${evidencias ? `<a href="${evidencias}">Ver Evidências</a>` : 'Nenhuma evidência anexada'}</p>
    `,
  };
